...
class Controller extends BaseController
{
    ...
    protected function getCryptocurrencyList()
    {
        ...
        for ($i = 0; $i < count($cryptocurrency_list); $i++) {
            ...
            else if ($cryptocurrency_list[$i]['currency'] == '8CO')
                $cryptocurrency_list[$i]['ico'] = asset('/icons/8co.png');
            else if ($cryptocurrency_list[$i]['currency'] == 'JCC')
                $cryptocurrency_list[$i]['ico'] = asset('/icons/jcc.png');
            else
                $cryptocurrency_list[$i]['ico'] = asset('/icons/btc.svg');
        }
...
//            $cryptocurrency_list[$i]['cashback'] = _number_format($cryptocurrency_list[$i]['cashback'], $cryptocurrency_list[$i]['rate_decimals']);
    ...
    protected function getBalance()
    {
        ...
        try {
            $balance_list = UserBalance::leftjoin('ct_crypto_settings', 'ct_crypto_settings.currency', '=', 'ct_users_balance.currency')
                ->where('ct_users_balance.user_id', $user->id)
                ->where('ct_users_balance.status', config('constants.balance_status.valid'))
                ->select('ct_users_balance.user_id', 'ct_users_balance.currency', 'ct_users_balance.balance', 'ct_crypto_settings.rate_decimals')
                ->get()->toArray();
        } catch (QueryException $e) {
            Log::error($e->getMessage());
            return $balance_list;
    ...
    protected function getBalanceFromCurrency($currency)
    {
        ...
        try {
            $balance_info = UserBalance::leftjoin('ct_crypto_settings', 'ct_crypto_settings.currency', '=', 'ct_users_balance.currency')
                ->where('ct_users_balance.user_id', $user->id)
    ...
    public static function getCurrencyList()
    {
        ...
        for ($i = 0; $i < count($currency_list); $i++) {
            ...
            else if ($currencies[0] == '8CO')
                $currency_list[$i]['ico'] = asset('/icons/8co.svg');
            else if ($currencies[0] == 'JCC')
                $currency_list[$i]['ico'] = asset('/icons/jcc.svg');
            else
                $currency_list[$i]['ico'] = asset('/icons/btc.svg');
        }
